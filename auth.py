...
def login_gestor(email, senha):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        "SELECT id, nome, senha FROM gestores WHERE email = %s",
        (email,)
    )
    row = cur.fetchone()
    cur.close()
    ...
    if not row:
        return None, None, "Email nao encontrado"

    id_gestor, nome, senha_hash = row

    if not bcrypt.checkpw(senha.encode(), senha_hash.encode()):
        return None, None, "Senha incorreta"

    return id_gestor, nome, "OK"
